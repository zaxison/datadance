...
- **出现动效**: 所有的 Tooltip 出现时需要附带由透明度 0 变到 1，同时由大小 `80%` 变到 `100%` 的渐显放大效果，动画时间建议在 `150ms`。对于固定挂载的元素可使用 CSS transition（`scale-80 -> scale-100`），对于 React Portal 动态挂载的元素，需使用自定义的 `@keyframes zoom-in-fade` 来触发入场动画。
